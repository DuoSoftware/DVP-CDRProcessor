...
var GetResourceStatusListWithACWDownload = function(startTime, endTime, statusList, agents, companyId, tenantId, callback)
{
    ...
    try
    {
        var defaultQuery = {where :[{CompanyId: companyId, TenantId: tenantId,$or:[{StatusType: 'ResourceStatus'},{Reason:"CALL"},{Reason:"CHAT"}], createdAt: {between:[startTime, endTime]}}], order: ['createdAt','ResourceId'], include: [{model: dbModel.ResResource, as: 'ResResource'}]};


        if(statusList && statusList.length > 0)
        {
            defaultQuery ={where :[{CompanyId: companyId, TenantId: tenantId, createdAt: {between:[startTime, endTime]}}], order: ['createdAt','ResourceId'], include: [{model: dbModel.ResResource, as: 'ResResource'}]};
            defaultQuery.where[0].$or = [];
